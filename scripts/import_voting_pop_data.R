...
# Now import reg rates by state

url = 'https://www2.census.gov/programs-surveys/cps/tables/time-series/voting-historical-time-series/a5b.xlsx'
...
assert_that(
  status$status_code == 200,
  typeof(status$content) == 'raw',
  length(status$content) > 1
)

# Fetch the data

curl_download(url = url, destfile = 'input/a5b.xlsx')

# Check that the data were downloaded correctly

assert_that(
  file.exists('input/a5b.xlsx')
)
